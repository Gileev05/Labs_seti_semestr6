...
const ChartDraw = (props) => {
    ...
    const scaleY = useMemo(() => {
        ...
        props.data.forEach(d => {
            if (showMax) currentValues.push(d.values[1]);
            if (showMin) currentValues.push(d.values[0]);
        });

        const [currentMin, currentMax] = d3.extent(currentValues);

        return d3
            .scaleLinear()
            .domain([currentMin - 0.1, currentMax])
            .range([height - 70, 0]);
    }, [props.data, showMax, showMin]);
